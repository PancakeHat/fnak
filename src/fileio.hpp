...
// Loads anims from a directory
// ANIMS DO NOT SUPPORT THE PACK SYSTEM
// only searches for files marked as .anim
void LoadAnimsFromDir(std::string animDir, std::vector<Anim>& anims, ErrorHandler& eh)
{
    // make path look pretty
    std::replace( animDir.begin(), animDir.end(), '\\', '/');

    // check if dirs exist
    if(!std::filesystem::exists(std::filesystem::path(animDir)))
    {
        ThrowNewError(std::format("Can't find anim directory {}", animDir), ERROR_FATAL, false, eh); 
        return;
    }
    ...
    for(const auto & entry : std::filesystem::directory_iterator("./assets"))
    {
        std::string s = entry.path().string();
        std::replace( s.begin(), s.end(), '\\', '/');
        if(StringEndsIn(s, ".anim"))
        {
            std::string id = s;
            id.erase(id.begin(), id.begin() + 9);
            id = RemoveFileEnding(id);
            
            LoadAnimToVector(s, id, anims, eh);

            std::cout << "GAME: Registered anim " << id << "\n";
        }
    }
}
